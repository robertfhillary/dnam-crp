...
# Run training 
lasso.cv <- cv.biglasso(methx, y, family="gaussian", alpha = 1, ncores = 8, nfolds = 20) # cross validation to get best lambda
fit <- biglasso(methx, y, family = "gaussian", alpha = 1, ncores = 8, lambda = lasso.cv$lambda.min) # model fit 
