...
###########################################################
### We will just plot Wielscher-PCA + Measured CRP for now ########
########################################################### 

# Plot Betas against each other for Wielscher-PCA vs CRP 
l3.p=l3 #for supp figure 
l3=l3[which(l3$Predictor %in% c("Measured CRP","Wielscher-PCA")),]
...
l3$Predictor=factor(l3$Predictor,levels=c("Measured CRP","Wielscher-PCA"))
...
g=ggplot(l3,aes(y=Beta, x=Trait, group = factor(Predictor),color=factor(Predictor))) + 
  ...
  geom_errorbar(position=position_dodge(width=0.7),aes(ymin = LCI, ymax = HCI), width = 0.05,
                colour = "gray50") + 
  ylab("Standardised Beta [95% Confidence Interval]")+ xlab ("")+ theme_scientific() + scale_color_manual(values=t[c(1,5)],name="Predictor") + 
...
# Subset to Wielscher-PCA and Measured CRP
l4=l4[l4$Predictor %in% c("Wielscher-PCA","Measured CRP"),]
l4$Trait=factor(l4$Trait,levels=c("hichol_w2","hibp_w2","cvdhist_w2","stroke_w2","diab_w2"))
l4$Predictor=factor(l4$Predictor,levels=c("Measured CRP","Wielscher-PCA"))
# Graph stage 
...
g2=ggplot(l4,aes(y=Odds, x=Trait, group = factor(Predictor),color=factor(Predictor))) + 
  ...
  ylab("Odds Ratio [95% Confidence Interval]")+ xlab ("")+ theme_scientific() + scale_color_manual(values=t[c(1,5)],name="Predictor") + 
...
# Subset to Wielscher-PCA and Measured CRP
l5=l5[l5$Predictor %in% c("Wielscher-PCA","Measured CRP"),]
l5$Predictor=factor(l5$Predictor,levels=c("Measured CRP","Wielscher-PCA"))
# Graph stage 
...
g3=ggplot(l5,aes(y=HR, x=Trait, group = factor(Predictor),color=factor(Predictor))) + 
  ...
  geom_errorbar(position=position_dodge(width=0.5),aes(ymin = LCI, ymax = HCI), width = 0.05,
                colour = "gray50")+
  ylab("Hazard Ratio [95% Confidence Interval]")+ xlab ("")+ theme_scientific() + scale_color_manual(values=t[c(1,5)],name="Predictor") + 
  geom_hline(yintercept = 1, linetype = "dotted")+ scale_y_continuous(limits = c(0,3), breaks = c(0,0.5,1,1.5,2,2.5,3))+ theme(axis.text=element_text(size=11),axis.title=element_text(size=12))+coord_flip()+
  facet_wrap(~Graph)+theme(strip.background = element_rect(fill = "gray95"),strip.text=element_text(size=11)) +  guides(color = "none")
print(g3)
dev.off() 
...
# Combined plot
pdf("/Cluster_Filespace/Marioni_Group/Rob/CRP/Plots/Figure4_combined_wielscher_pca.pdf",width=14,height=9)
